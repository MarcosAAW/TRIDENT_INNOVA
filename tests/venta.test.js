...
describe('Ventas API (integración)', () => {
  ...
  test('factura una venta USD con descuento y entrega inicial enviando total neto, anticipo y saldo financiado correctos', async () => {
    ...
    expect(payloadFactpy?.items).toHaveLength(2);
    expect(Number(payloadFactpy?.items?.[0]?.precioUnitario)).toBeCloseTo(950, 4);
    expect(Number(payloadFactpy?.items?.[0]?.descuento)).toBeCloseTo(0, 8);
    expect(Number(payloadFactpy?.items?.[0]?.cantidad)).toBeCloseTo(1, 8);
    expect(Number(payloadFactpy?.items?.[0]?.precioTotal)).toBeCloseTo(950, 4);
    expect(Number(payloadFactpy?.items?.[0]?.baseGravItem)).toBeCloseTo(863.63636364, 8);
    expect(Number(payloadFactpy?.items?.[0]?.liqIvaItem)).toBeCloseTo(86.36363636, 8);
    expect(Number(payloadFactpy?.items?.[1]?.cantidad)).toBeCloseTo(1, 8);
    expect(Number(payloadFactpy?.items?.[1]?.precioTotal)).toBeCloseTo(950, 4);
